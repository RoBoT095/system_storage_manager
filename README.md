...
print(dir.name); // name of the selected folder
...
await _ssm.pickFiles(); // returns a list of [FileItem]s that were selected from file picker

// Getting a list of files/folder in a folder
await _ssm.listFiles(dir.uri, showHidden: false); // returns list of [FileItem]s and skips any that start with a period (.) aka hidden files
...
await _ssm.parentUri(file.uri); // If file is in "../Documents/file.txt" then output will be "../Documents"
...
/// Moving
file = await _ssm.move(fileCopy.uri, dir.uri); // returns the file/folder with its new location, note that this currently just overwrites the `file` with the new `fileCopy` in code and in device storage, careful what you do with this

// Now the juicy parts

/// Reading files
final String fileText = await _ssm.readAsString(file.uri); // returns data as a [String]
final Uint8List fileBytes = await _ssm.readAsBytes(file.uri); // returns data as a [List<int>], you can use as [Uint8List]
